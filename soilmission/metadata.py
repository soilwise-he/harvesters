...
for rec in sorted(recs):    
    ...
    matches = matches2 = matches3 = None
    if jsonResponse:
        matches = jsonResponse.get('response',{})
    if matches:
        matches2 = matches.get('results',{})
    if not matches2: # happens if query result is empty
        matches3=[]
    else:
        matches3 = matches2.get('result',[])
    if matches3:
        print('project',code,'nr of records',len(matches3))
        for r in matches3:
            ...
            if isinstance(title, list):
                #print('Warning: failed parsing as str',title)
                title = title[0]
            title = title.get('$','')
            pid = None
            for p in m.get('pid',[]):
            ...
            if pid:
                ...
                # hasrec = dbQuery(f"select identifier from harvest.items where identifier = '{pid}'")
                # if not hasrec:
        ...
        print('Warning, failed',code,jsonResponse)
